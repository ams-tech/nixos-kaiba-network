...
let
  ...
  cfg = config.services.kaiba-provisioning-audit;
  defaultPackage = (import ../packages.nix { inherit pkgs lib; }).audit;
  loopbackAddresses = [
    "127.0.0.1"
    "::1"
  ];
  ...
  args = [
    (getExe' cfg.package "kaiba-provision-audit")
    "--listen"
    listenEndpoint
    "--state"
    statePath
...
in
{
  options.services.kaiba-provisioning-audit = {
    enable = mkEnableOption "the local Kaiba append-only provisioning audit reference service";

    package = mkOption {
      type = types.package;
      default = defaultPackage;
      defaultText = lib.literalExpression "the kaiba-provision-audit package from the provisioning source tree";
      example = lib.literalExpression "inputs.kaiba-provisioning.packages.\${pkgs.system}.kaiba-provision-audit";
      description = "Package containing bin/kaiba-provision-audit.";
    };
    ...
    listenAddress = mkOption {
      ...
      example = "::1";
      description = ''
        Concrete IP address on which the reference audit service listens. The
    ...
    port = mkOption {
      type = types.port;
      default = 8092;
      description = "TCP port for the provisioning audit service.";
    };
    ...
    tlsCertificateFile = mkOption {
      ...
      default = null;
      example = "/run/keys/kaiba-audit-server.crt";
      description = ''
    ...
    tlsPrivateKeyFile = mkOption {
      type = types.nullOr types.str;
      default = null;
      example = "/run/keys/kaiba-audit-server.key";
      description = ''
    ...
    stateFile = mkOption {
      ...
      default = "audit.json";
      description = "Append-only audit snapshot filename within stateDirectory.";
    };
  };
  ...
  config = mkIf cfg.enable {
    assertions = [
      {
        ...
        message = ''
          services.kaiba-provisioning-audit.listenAddress must be an IPv4 or
      ...
      }
      {
        ...
        message = ''
          services.kaiba-provisioning-audit TLS requires absolute, non-store
    ...
    systemd.services.kaiba-provisioning-audit = {
