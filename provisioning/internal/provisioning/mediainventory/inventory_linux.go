...
func (inventory SystemInventory) defaults() SystemInventory {
	...
	if inventory.BootIDPath == "" {
		inventory.BootIDPath = "/proc/sys/kernel/random/boot_id"
	}
	return inventory
}
...
func (inventory SystemInventory) inspectDevice(requestedPath string) (TargetFacts, error) {
	...
	bootIDBytes, err := os.ReadFile(inventory.BootIDPath)
	if err != nil {
		return TargetFacts{}, fmt.Errorf("read kernel boot identity: %w", err)
	}
	bootID := strings.TrimSpace(string(bootIDBytes))
	if !bootIDPattern.MatchString(bootID) || len(bootIDBytes) > 128 {
		return TargetFacts{}, errors.New("kernel boot identity is not one canonical lowercase UUID")
	}
...
// BlockDeviceSize reads the capacity of an already opened block-device
// descriptor. It is exported only for the legacy adapter compatibility shim.
func BlockDeviceSize(file *os.File) (uint64, error) { return blockDeviceSize(file) }
...
// BlockDeviceDiskSequence reads Linux's boot-local attachment sequence from
// an already opened block-device descriptor.
func BlockDeviceDiskSequence(file *os.File) (uint64, error) {
	return blockDeviceDiskSequence(file)
}
