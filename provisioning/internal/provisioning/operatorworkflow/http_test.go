...
func TestHTTPControlClientCurrentClaimPreflightUsesFixedTypedOperation(t *testing.T) {
	want := controlplane.CurrentClaimPreflightRequest{
	...
	httpClient := &http.Client{Transport: roundTripFunc(func(request *http.Request) (*http.Response, error) {
		...
		var got controlplane.CurrentClaimPreflightRequest
		if err := controlplane.DecodeStrict(command.Request, &got); err != nil {
			t.Fatal(err)
		}
		if !reflect.DeepEqual(got, want) {
			t.Fatalf("typed current-claim preflight = %#v, want %#v", got, want)
		}
		...
		if len(fields) != 0 {
			t.Fatalf("current-claim preflight carried unexpected fields: %v", fields)
		}
		return &http.Response{
			StatusCode: http.StatusOK, Header: http.Header{"Content-Type": []string{"application/json"}},
			Body: io.NopCloser(strings.NewReader(string(responseBody))),
	...
	if _, err := client.PreflightCurrentClaim(context.Background(), want); err != nil {
		t.Fatal(err)
	}
}
