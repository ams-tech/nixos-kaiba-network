...
- A device knows only its update endpoint, its mTLS identity, and its own
  addresses. It never knows P0, P1, or a DNS-provider API.
...
Production deployments must provision certificate keys and TSIG material at
runtime paths (the module examples use `/run/credentials/...`) and order the
services after those provisioning units. The PKI and TSIG fixtures generated by
`tests/integration` deliberately enter the Nix store and are named and scoped
as test-only; they are unsuitable for deployment.
...
state, fleet-scale load and sharding, Raspberry Pi image and secure-boot work,
deployment, production secret distribution, and monitoring.
