...
class ReportRendererTest(unittest.TestCase):
    ...
    def test_signed_release_manifest_contract_is_visible_in_every_report_format(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            provisioning = json.loads(
                (REPO_ROOT / "tests" / "provisioning" / "report-input.json").read_text(encoding="utf-8")
            ...
            contract_rows = [
                (item["system"], item["status"])
                for item in rendered["automated"]["checks"]
                if item["id"] == "signed-release-manifest-contract"
            ]
            self.assertEqual(
                [("aarch64-linux", "not-observed"), ("x86_64-linux", "passed")],
                contract_rows,
            ...
            markdown = (output / "index.md").read_text(encoding="utf-8")
            self.assertIn("| NOT OBSERVED | `aarch64-linux` | `signed-release-manifest-contract` |", markdown)
            self.assertIn("| PASSED | `x86_64-linux` | `signed-release-manifest-contract` |", markdown)

            page = (output / "index.html").read_text(encoding="utf-8")
            self.assertEqual(2, page.count("<code>signed-release-manifest-contract</code>"))

            junit = ET.parse(output / "junit.xml")
            cases = {
                case.get("classname"): case
                for case in junit.findall('.//testcase[@name="signed-release-manifest-contract"]')
            }
            self.assertEqual(
                {"provisioning.aarch64-linux", "provisioning.x86_64-linux"},
                set(cases),
